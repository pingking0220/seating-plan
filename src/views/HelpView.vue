  ...
  <div class="help">
    ...
    <section :id="sections[3].id" class="panel sec">
      ...
      <ul>
        ...
        <li>座位格太小看不清楚？工具列的 <strong>🔍−／🔍＋</strong> 可縮放座位表（預設 125%）；「<strong>📐 尺寸</strong>」可自訂座位格的寬、高與字體大小，拉滑桿即時預覽（畫布以真實像素呈現，加寬就是真的變寬；座位間距與走道寬度固定，不隨座位尺寸縮放），設定隨座位表儲存、列印同步套用。</li>
      </ul>
      <h4>自動排位</h4>
      <ul>
        <li>「🎲 自動排位」會綜合需求標籤、人際關係、分組品質自動安排，幾乎瞬間完成。</li>
        <li>「換一個方案」產生另一種排法，可以多按幾次挑喜歡的。</li>
        <li>「⚖️ 規則」裡每條規則都能開關、調權重（1–10，越高越優先）。例如「男女不同排」讓每一直行同性別、左右交錯（男女男女…）；「往前坐不留空」以教室前方為基準，每一直行由前往後坐滿、空位留在行尾；「每排人數平均」讓各直行人數差不超過兩人；「座號順序（由左至右／由右至左）」預設關閉，開啟後 1 號從第一直行最前面開始依序入座（兩個方向擇一）。座號順序與「男女不同排」可以併用：男生排按男生座號、女生排按女生座號、左右交錯；「男女不同排」旁的下拉選單可指定「男生先」或「女生先」（以最左排為準），預設自動。不需要的規則把它關掉即可。</li>
      ...
      <h4>加減分模式</h4>
      <p>
        工具列按「<strong>⭐ 加減分</strong>」開啟：點座位上的學生就直接加 1 分（或切到「−1」扣分），
        分數顯示在座位右下角的徽章（綠色正分、紅色負分），全班可「歸零」重新計。
        加減分存在班級名單上，同班的每張座位表看到的是同一份分數；開啟期間編排功能暫停，再按一次即關閉。
        上課獎懲、小組競賽計分都好用 — 投影座位表出來直接點。
      </p>
      <h4>公平輪替</h4>
      <p>
    ...
    <section :id="sections[6].id" class="panel sec">
      ...
      <dl>
        ...
        <dt>可以把一個班的規則設定套用到其他班嗎？</dt>
        <dd>可以。座位表列表的「📑 批次建立」（或某張座位表的「套用到其他班」）：選一張範本座位表，勾選班級，一鍵建立 — 新座位表沿用範本的教室佈局、規則設定與版面設定（座位尺寸、字體、縮放），並可立即依各班名單自動排位。預設會覆蓋「同班級＋同教室」的舊座位表（保留原名稱與鎖定座位），要另存新檔就取消勾選「覆蓋」。</dd>
        <dt>可以多班共用一種教室佈局嗎？</dt>
